...
enum FotaFwRole { repeater, roomServer }

String fotaRoleInfix(FotaFwRole r) =>
    r == FotaFwRole.repeater ? 'repeater' : 'room_server';

class FotaReleaseAsset {
  final String name;
  final String downloadUrl;
  const FotaReleaseAsset({required this.name, required this.downloadUrl});
}
...
/// the name does not match a known FOTA firmware role.
FotaAssetInfo? parseFotaAssetName(String name) {
  for (final role in FotaFwRole.values) {
    final marker = '_${fotaRoleInfix(role)}-v';
...
FotaReleaseAsset? selectFotaAsset(
    List<FotaReleaseAsset> assets, String device, FotaFwRole role) {
  FotaReleaseAsset? zip;
  for (final a in assets) {
    final info = parseFotaAssetName(a.name);
...
String fotaPackageFileName({
  required String device,
  required FotaFwRole role,
  required String currentVersion,
  required String targetVersion,
}) =>
    '${device}_${fotaRoleInfix(role)}_v${currentVersion}_to_v$targetVersion.fotapkg.json';

/// Newest-first comparator for dotted numeric versions ("1.17.0" before "1.16.2").
int compareFotaVersionsDesc(String a, String b) {
...
class FotaRelease {
  ...
  final String version;
  final String tag;
  final List<FotaReleaseAsset> assets;
  const FotaRelease({
    required this.role,
    required this.version,
    required this.tag,
...
class FotaFwCatalog {
  ...
  FotaReleaseAsset? assetFor(String device, String version) {
    final r = _release(version);
    if (r == null) return null;
    return selectFotaAsset(r.assets, device, role);
  }
}

FotaFwCatalog buildFotaCatalog({
  required FotaFwRole role,
  required List<FotaRelease> releases,
  required Set<String> nrfBoardNamesLower,
}) {
  final forRole = releases.where((r) => r.role == role).toList()
    ..sort((a, b) => compareFotaVersionsDesc(a.version, b.version));
  ...
  return FotaFwCatalog(role: role, releases: forRole, devices: sorted);
}
